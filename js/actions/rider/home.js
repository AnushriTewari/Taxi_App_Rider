...
export function fetchPrediction(queryString) {
  return (dispatch, getState) => {
    const url = `https://maps.googleapis.com/maps/api/place/queryautocomplete/json?key=AIzaSyAVdBLJFLW7q2HyJ1pqlqmvlTWZP7ZgjJs&input=${queryString}&location=${getState().rider.user.gpsLoc[0]},${getState().rider.user.gpsLoc[1]}&radius=50000`;
    fetch(url)
...
export function fetchAddressFromCoordinatesAsync(region) {
  return (dispatch, getState) =>
    fetch(
      `https:/maps.googleapis.com/maps/api/geocode/json?latlng=${region.latitude},${region.longitude}&key=${getState()
        .basicAppConfig.config.googleMapsApiKey}`,
      {
        method: "GET"
      }
    )
      .then(resp => resp.json())
      .then(address => dispatch(setCurrentAddress(address)))
      .catch(e => e);
}
